# The keys of a dictionary are very similar to a set. The main difference is that dictionary keys are ordered.
# Sets have no ordering.
# Items only appear once in a union set. The elements are unique.
# Sets are unordered. This means we cannot index a set. Items in a set don't have a fixed position.
# Python considers the sets to be equal if they contain the same items in them, the order isn't important.
# ############# Set Methods ######################
# clear() - clears every item in a set to leave an empty set.
# discard() - removes a specified item from the set. Nothing changes if the specified item is not in the set.
# remove() - removes a specified item from the set. If nothing is specified it returns an error, the programme crashes.
